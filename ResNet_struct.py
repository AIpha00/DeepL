...
class Block(collections.namedtuple('block', ['name', 'residual_unit', 'args'])):
    ##name: 这个残差学习模块的名称
    # residual_unit：是指创建这个残差学习模块用到的函数
    ##args: 这个残差学习模块的大小信息
...
def residual_unit(inputs, depth, depth_residual, stride, outputs_collections=None, scope=None):
    ...
    with tf.variable_scope(scope, 'residual_v2', [inputs]) as sc:
        ...
        ##使用slim.batch_norm进行BN(batch 归一化操作)
        preact = slim.batch_norm(inputs, activation_fn=tf.nn.relu, scope='preac')

        ###如果本快的depth值(depth)参数等于上一个块的depth，则考虑进行降采样操作
        ##如果depth值不等于depth_input，则使用conv2d函数使输入通道数和输出通道数一致

        ##直连层
        if depth == depth_input:
            # 如果stride等1， 则不进行降采样操作；如果stride不等于1，则使用max_pool2d
...
def resnet_v2(inputs, blocks, num_classes, reuse=None, scope=None):
    with tf.variable_scope(scope, 'resnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([residual_unit], outputs_collections=end_points_collection):
            ...
            ##在两个嵌套的for循环中调用residual_unit()函数堆砌ResNet的结构
            for block in blocks:
                # block.name分别为block1\2\3\4
                with tf.variable_scope(block.name, 'block', [net]) as sc:
                    ...
                    for i, tuple_value in enumerate(block.args):
                        # i的值从0开始，对于第一个unit，i需要+1
                        with tf.variable_scope("unit_%d" % (i + 1), values=[net]):
                            ##每一个元组都由3个数组成，将这3个数作为参数传递到block类的residual_unit参数中，在定义blocks时，这个参数就是函数residual_unit
            ...
            # 对net使用slim.batch_norm函数进行BN操作
            print(net)
            net = slim.batch_norm(net, activation_fn=tf.nn.relu, scope="postnorm")
            ##创建全局平均池化层
            net = tf.reduce_mean(net, [1, 2], name='pool5', keep_dims=True)
            ##如果定义了num_classes,则通过1x1卷积的方式获取数目为num_classes的输出
